...
const postCopyIntakeFile = process.argv[2] ?? "outputs/diablo4-delta-evidence-post-copy-intake/delta-evidence-post-copy-intake.json";
const outDir = process.argv[3] ?? "outputs/diablo4-delta-evidence-manual-review-gate";
...
const gateChecks = [
  ...
  },
  {
    ...
    finding: targetBlockers.includes("manual-review-required")
      ...
      : "Le bloqueur de revue humaine n'est pas present.",
  },
  {
    id: "not-approved",
...
const report = {
  ...
  schemaVersion: 1,
  mode: "delta-evidence-manual-review-gate-v1",
  source: {
    postCopyIntakeFile,
    outDir,
  },
  summary: {
    ...
    assessment: {
      kind: readyForReviewerDecision
        ? "delta-evidence-manual-review-gate-ready"
        : "delta-evidence-manual-review-gate-blocked",
      confidence: "high",
      promotionReady: false,
...
const outFile = path.join(outDir, "delta-evidence-manual-review-gate.json");
