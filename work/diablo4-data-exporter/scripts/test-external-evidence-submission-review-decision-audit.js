...
const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "d4-external-evidence-submission-review-decision-audit-"));
...
run("audit-external-evidence-submission-review-decision.js", [
  ...
  "inputs/external-evidence-submission-review-decision.json",
  blockedOutDir,
]);

const blocked = readJson(path.join(blockedOutDir, "external-evidence-submission-review-decision-audit.json"));
...
assertInvariant(fs.existsSync(path.join(blockedOutDir, "external-evidence-submission-review-decision.template.json")), "blocked audit should emit template");

const readyPackage = readJson("outputs/diablo4-external-evidence-submission-review-decision-package/external-evidence-submission-review-decision-package.json");
...
const approved = readJson(path.join(approvedOutDir, "external-evidence-submission-review-decision-audit.json"));
...
const rejected = readJson(path.join(rejectedOutDir, "external-evidence-submission-review-decision-audit.json"));
...
console.log(JSON.stringify({
  status: "external-evidence-submission-review-decision-audit-test-ok",
