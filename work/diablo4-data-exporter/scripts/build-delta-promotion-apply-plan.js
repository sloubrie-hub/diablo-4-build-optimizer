...
const applicationGateFile = process.argv[2] ?? "outputs/diablo4-delta-promotion-application-gate/delta-promotion-application-gate.json";
const outDir = process.argv[3] ?? "outputs/diablo4-delta-promotion-apply-plan";
...
const planChecks = [
  {
    ...
    finding: summary.manualApplyAllowed === true
      ? "La porte autorise la preparation d'une application manuelle."
      : "La porte d'application est fermee.",
  },
  {
...
const report = {
  ...
  schemaVersion: 1,
  mode: "delta-promotion-apply-plan-v1",
  source: {
    applicationGateFile,
    outDir,
  },
  summary: {
    ...
    assessment: {
      kind: applyPlanReady
        ? "delta-promotion-apply-plan-ready"
        : "delta-promotion-apply-plan-blocked",
      confidence: "high",
      promotionReady: false,
      finding: applyPlanReady
        ? "Le plan d'application explicite est pret, mais n'applique rien."
        : "Le plan d'application reste bloque.",
...
const outFile = path.join(outDir, "delta-promotion-apply-plan.json");
