...
const dryRunFile = process.argv[2] ?? "outputs/diablo4-delta-promotion-implementation-dry-run/delta-promotion-implementation-dry-run.json";
const outDir = process.argv[3] ?? "outputs/diablo4-delta-promotion-application-gate";
...
const gateChecks = [
  {
    ...
    finding: summary.patchPreviewReady === true
      ? "Le dry-run de promotion est pret."
      : "Le dry-run de promotion reste bloque.",
  },
  {
...
const report = {
  ...
  schemaVersion: 1,
  mode: "delta-promotion-application-gate-v1",
  source: {
    dryRunFile,
    outDir,
  },
  summary: {
    ...
    assessment: {
      kind: manualApplyAllowed
        ? "delta-promotion-application-gate-ready-manual-only"
        : "delta-promotion-application-gate-blocked",
      confidence: "high",
      promotionReady: false,
      finding: manualApplyAllowed
        ? "Une application manuelle separee peut etre preparee, sans ecriture automatique ici."
        : "La porte d'application reste fermee.",
  ...
  patchPreview,
  applyContract: {
    allowedNextStep: "delta-promotion-apply-explicit",
    requiredBeforeApply: [
...
const outFile = path.join(outDir, "delta-promotion-application-gate.json");
