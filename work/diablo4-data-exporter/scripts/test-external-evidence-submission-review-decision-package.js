...
const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "d4-external-evidence-submission-review-decision-"));
...
run("build-external-evidence-submission-review-decision-package.js", [
  "outputs/diablo4-external-evidence-submission-manual-review-gate/external-evidence-submission-manual-review-gate.json",
  blockedOutDir,
]);

const blocked = readJson(path.join(blockedOutDir, "external-evidence-submission-review-decision-package.json"));
const blockedMarkdown = fs.readFileSync(path.join(blockedOutDir, "external-evidence-submission-review-decision-package.md"), "utf8");
...
assertInvariant(blockedMarkdown.includes("Decision reviewer soumission preuve externe 1663210"), "blocked package should emit markdown");

const readyGate = readJson("outputs/diablo4-external-evidence-submission-manual-review-gate/external-evidence-submission-manual-review-gate.json");
...
readyGate.targetCandidate = {
  ...
  domain: "external-evidence-submission",
  assetId: 1663210,
...
const ready = readJson(path.join(readyOutDir, "external-evidence-submission-review-decision-package.json"));
const readyMarkdown = fs.readFileSync(path.join(readyOutDir, "external-evidence-submission-review-decision-package.md"), "utf8");
...
console.log(JSON.stringify({
  status: "external-evidence-submission-review-decision-package-test-ok",
