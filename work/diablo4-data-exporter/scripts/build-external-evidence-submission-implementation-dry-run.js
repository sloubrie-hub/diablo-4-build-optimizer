...
const promotionAuditFile = process.argv[2] ?? "outputs/diablo4-external-evidence-submission-promotion-audit/external-evidence-submission-promotion-audit.json";
const targetDatasetFile = process.argv[3] ?? "outputs/diablo4-target-dataset/target-dataset.json";
const outDir = process.argv[4] ?? "outputs/diablo4-external-evidence-submission-implementation-dry-run";
...
const dryRunChecks = [
  {
    id: "external-promotion-audit-ready",
    status: summary.readyForPromotionImplementation === true ? "passed" : "failed",
    finding: summary.readyForPromotionImplementation === true
      ? "L'audit de promotion externe autorise un dry-run d'implementation."
      : "L'audit de promotion externe reste bloque.",
  },
  {
...
const report = {
  ...
  schemaVersion: 1,
  mode: "external-evidence-submission-implementation-dry-run-v1",
  source: {
    promotionAuditFile,
    targetDatasetFile,
    outDir,
  ...
  summary: {
    ...
    assessment: {
      kind: patchPreviewReady
        ? "external-evidence-submission-implementation-dry-run-ready"
        : "external-evidence-submission-implementation-dry-run-blocked",
      confidence: "high",
      promotionReady: false,
      finding: patchPreviewReady
        ? "Le dry-run externe de patch est pret, mais aucune ecriture n'est effectuee."
        : "Le dry-run externe d'implementation reste bloque.",
      nextAction: patchPreviewReady
        ? "Creer une etape d'application externe separee avec validation de regression complete."
...
const outFile = path.join(outDir, "external-evidence-submission-implementation-dry-run.json");
