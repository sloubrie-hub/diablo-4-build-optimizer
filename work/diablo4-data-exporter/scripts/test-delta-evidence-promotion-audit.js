...
const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "d4-delta-evidence-promotion-audit-"));
...
run("build-delta-evidence-promotion-audit.js", [
...
const blocked = readJson(path.join(blockedOutDir, "delta-evidence-promotion-audit.json"));
assertInvariant(blocked.summary.readyForPromotionImplementation === false, "real promotion audit should remain blocked");
assertInvariant(blocked.summary.failedChecks > 0, "blocked promotion audit should expose failed checks");
assertInvariant(blocked.summary.failedGateIds.includes("sf32-field"), "real promotion audit should keep SF_32 blocked");
assertInvariant(blocked.summary.canModifyReliableDps === false, "blocked promotion audit must not modify reliable DPS");
assertInvariant(blocked.summary.promotionReady === false, "blocked promotion audit must not auto-promote");

const approvedDecisionAudit = readJson("outputs/diablo4-delta-evidence-review-decision-audit/delta-evidence-review-decision-audit.json");
...
readyReliableGates.gates = readyReliableGates.gates.map((gate) => ({
  ...gate,
  status: "passed",
  reason: `Synthetic passed gate for promotion audit test: ${gate.id}`,
}));
...
const ready = readJson(path.join(readyOutDir, "delta-evidence-promotion-audit.json"));
assertInvariant(ready.summary.readyForPromotionImplementation === true, "synthetic promotion audit should open implementation only");
...
assertInvariant(ready.summary.acceptedForBridge === false, "ready promotion audit must not accept bridge");
assertInvariant(ready.summary.canModifyReliableDps === false, "ready promotion audit must not modify reliable DPS");
assertInvariant(ready.summary.canUseForReliableDps === false, "ready promotion audit must not allow reliable DPS");
assertInvariant(ready.summary.canUseForRanking === false, "ready promotion audit must not allow ranking");
assertInvariant(ready.summary.promotionReady === false, "ready promotion audit must not auto-promote");
assertInvariant(ready.auditChecks.every((check) => check.status === "passed"), "ready fixture should pass promotion audit checks");
...
console.log(JSON.stringify({
  status: "delta-evidence-promotion-audit-test-ok",
