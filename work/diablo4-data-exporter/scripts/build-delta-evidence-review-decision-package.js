...
const reviewGateFile = process.argv[2] ?? "outputs/diablo4-delta-evidence-manual-review-gate/delta-evidence-manual-review-gate.json";
const outDir = process.argv[3] ?? "outputs/diablo4-delta-evidence-review-decision-package";
...
function renderDecisionMarkdown(report) {
  ...
  return [
    "# Decision reviewer delta 1663210",
    "",
...
const report = {
  ...
  schemaVersion: 1,
  mode: "delta-evidence-review-decision-package-v1",
  source: {
    reviewGateFile,
    outDir,
  },
  summary: {
    ...
    assessment: {
      kind: readyForDecisionInput
        ? "delta-evidence-review-decision-package-ready"
        : "delta-evidence-review-decision-package-blocked",
      confidence: "high",
      promotionReady: false,
...
const outFile = path.join(outDir, "delta-evidence-review-decision-package.json");
const markdownFile = path.join(outDir, "delta-evidence-review-decision-package.md");
