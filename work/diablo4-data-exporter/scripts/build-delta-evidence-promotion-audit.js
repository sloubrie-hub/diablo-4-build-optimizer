...
const decisionAuditFile = process.argv[2] ?? "outputs/diablo4-delta-evidence-review-decision-audit/delta-evidence-review-decision-audit.json";
...
const outDir = process.argv[5] ?? "outputs/diablo4-delta-evidence-promotion-audit";
...
const auditChecks = [
  {
    ...
    finding: decisionAudit.summary?.decisionAcceptedForPromotionReview === true
      ? "La decision reviewer approved est auditee."
      : "Aucune decision approved auditee n'ouvre la promotion.",
  },
  {
...
const report = {
  ...
  schemaVersion: 1,
  mode: "delta-evidence-promotion-audit-v1",
  source: {
    decisionAuditFile,
    reliableGatesFile,
  ...
  summary: {
    ...
    assessment: {
      kind: readyForPromotionImplementation
        ? "delta-evidence-promotion-audit-ready-for-implementation"
        : "delta-evidence-promotion-audit-blocked",
      confidence: "high",
      promotionReady: false,
      finding: readyForPromotionImplementation
        ? "La promotion peut etre implementee dans une etape separee, sans modification automatique ici."
        : "La promotion fiable reste bloquee.",
  ...
  implementationContract: {
    allowedNextStep: "delta-promotion-implementation-dry-run",
    requiredBeforeWrite: [
...
const outFile = path.join(outDir, "delta-evidence-promotion-audit.json");
