...
const decisionPackageFile = process.argv[2] ?? "outputs/diablo4-delta-evidence-review-decision-package/delta-evidence-review-decision-package.json";
const decisionInputFile = process.argv[3] ?? "inputs/delta-evidence-review-decision.json";
const outDir = process.argv[4] ?? "outputs/diablo4-delta-evidence-review-decision-audit";
...
const templateFile = path.join(outDir, "delta-evidence-review-decision.template.json");
...
const report = {
  ...
  schemaVersion: 1,
  mode: "delta-evidence-review-decision-audit-v1",
  source: {
    decisionPackageFile,
    decisionInputFile,
  ...
  summary: {
    ...
    assessment: {
      kind: decisionAcceptedForPromotionReview
        ? "delta-evidence-review-decision-approved-for-promotion-audit"
        : decisionRejected
          ? "delta-evidence-review-decision-rejected"
          : "delta-evidence-review-decision-audit-blocked",
      confidence: "high",
      promotionReady: false,
...
const outFile = path.join(outDir, "delta-evidence-review-decision-audit.json");
